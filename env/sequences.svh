...
class pfe_parallel_sequence extends pfe_base_sequence;
  `uvm_object_utils(pfe_parallel_sequence)
  function new(string name = "pfe_parallel_sequence"); super.new(name); endfunction
  task body();
    for (int lat = 0; lat < 4; lat++) begin
      repeat (32) begin
        pfe_input_cycle_item req = make_random_beat(full_mask());

class pfe_scoreboard extends uvm_component;
  ...
  function void write_pktin(pfe_packet observed);
    ...
    if (pkt.dp != 0) begin
      ...
      if (!packet_db.exists(pkt.dependency_seq)) begin
        ...
        // Dependency results are deterministic, so chains can be calculated
        // recursively at acceptance without observing the integrated FE.
  ...
  function void write_pktout(pfe_out_packet out_pkt);
    ...
    if (expected.output_seen)
      `uvm_error("DUP_PKTOUT",
        $sformatf("seq=%0d was output more than once", next_output_seq))
  ...
  task wait_for_output_count(longint unsigned expected_count,
    ...
    if (timed_out)
      `uvm_error("DRAIN_TIMEOUT",
        $sformatf("after %0d cycles expected_outputs=%0d actual_outputs=%0d accepted=%0d",
  ...
  function real output_utilization();
    if (observed_cycles == 0 || cfg.lane_num == 0) return 0.0;
    return real'(output_slots) /
           real'(observed_cycles * longint'(cfg.lane_num));
  endfunction
  ...
  function void report_phase(uvm_phase phase);
    ...
    if (!uvm_config_db#(string)::get(this, "", "tc_name", test_name)) begin
      if (!$value$plusargs("PFE_TC_NAME=%s", test_name))

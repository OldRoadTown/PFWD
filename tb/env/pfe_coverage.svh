class pfe_coverage extends uvm_component;
  ...
  uvm_analysis_imp_cov_fe_req #(pfe_fe_req, pfe_coverage) fe_req_imp;
  ...
  covergroup fe_cg with function sample(
    ...
    option.per_instance = 1;
    cp_engine: coverpoint engine_lane {
      bins lanes[] = {[0:`PFE_LANE_NUM-1]};
    }
    cp_latency: coverpoint latency { bins all[] = {[0:3]}; }
    cp_dp_vld: coverpoint dp_vld;
    cp_parallel: coverpoint parallelism {
      bins counts[] = {[1:`PFE_LANE_NUM]};
    }
    engine_x_latency: cross cp_engine, cp_latency;
    latency_x_dep: cross cp_latency, cp_dp_vld;
  endgroup
  ...
  function void write_cov_fe_req(pfe_fe_req req);
    fe_cg.sample(req.engine_lane, req.latency, req.dp_vld,
                 req.beat_parallelism);
  endfunction

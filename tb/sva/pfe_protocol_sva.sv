...
module pfe_protocol_sva #(
  ...
  a_reset_quiet: assert property (
    @(posedge vif.clk) !vif.rst_n |->
      (vif.pkt_out_vld == '0 && vif.fwd_pkt_data_vld == '0 &&
       vif.fwd_pkt_dp_vld == '0)
  ...
  a_known_output_valids: assert property (
    @(posedge vif.clk) disable iff (!vif.rst_n)
      !$isunknown({vif.pkt_in_bkpr, vif.pkt_out_vld,
                   vif.fwd_pkt_data_vld, vif.fwd_pkt_dp_vld})
  ...
  generate
    for (genvar lane = 0; lane < LANE_NUM; lane++) begin : g_lane_sva
      ...
      a_fe_request_known: assert property (
        @(posedge vif.clk) disable iff (!vif.rst_n)
          vif.fwd_pkt_data_vld[lane] |->
            !$isunknown({vif.fwd_pkt_data[lane], vif.fwd_pkt_lat[lane],
                         vif.fwd_pkt_dp_vld[lane]})
      ) else $error("FEIN lane %0d contains X/Z", lane);

      a_dp_only_with_request: assert property (
        @(posedge vif.clk) disable iff (!vif.rst_n)
          vif.fwd_pkt_dp_vld[lane] |-> vif.fwd_pkt_data_vld[lane]
      ) else $error("FEIN lane %0d asserted dp_vld without data_vld", lane);

      a_dp_data_known: assert property (
        @(posedge vif.clk) disable iff (!vif.rst_n)
          vif.fwd_pkt_dp_vld[lane] |->
            !$isunknown(vif.fwd_pkt_dp_data[lane])
      ) else $error("FEIN lane %0d dp_data contains X/Z", lane);

      a_fe_response_known: assert property (
        @(posedge vif.clk) disable iff (!vif.rst_n)
          vif.fwded_pkt_data_vld[lane] |->
            !$isunknown(vif.fwded_pkt_data[lane])
      ) else $error("FEOUT lane %0d contains X/Z", lane);
  ...
  c_all_fe_parallel: cover property (
    @(posedge vif.clk) disable iff (!vif.rst_n)
      (&vif.fwd_pkt_data_vld));
